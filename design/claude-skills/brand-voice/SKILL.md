---
name: brand-voice
description: Enforce Banking Digital platform brand voice rules — capitalization, punctuation, component-specific copy formats, and TH/EN style conventions. Use alongside ux-writing skill to ensure all copy matches the brand guideline.
argument-hint: "<copy to review, component type, or 'audit'>"
allowed-tools:
...
Enforce the official UX writing guideline for the Banking Digital platform. This skill carries the brand-specific rules; pair it with the **ux-writing** skill which provides the universal writing workflow and MCP integration.
...
### Inline Errors
- **Field-level**: Short, sentence case, state what is wrong (e.g., "Incorrect PromptPay ID" / "หมายเลขพร้อมเพย์ไม่ถูกต้อง")
- **Form-level**: Action-oriented, tell how to fix (e.g., "Unable to proceed. Please complete all fields.")
...
- **Explanatory**: 1-2 sentences max (~80 chars), sentence case. Tell user what to do, not what the element is. Do not repeat the label.
...
- TH: กระชับ ตรงประเด็น ไม่ขึ้นต้นด้วยคำนำ
...
- **Acronyms** — only if common or previously explained (e.g., LG, EIPP)
...
## Output Format — Write
    ...
    **TH**: [Copy following all rules]
    ...
    ### Rules Applied
    - [Which capitalization, punctuation, component format rules were applied]
    
    ### Alternatives
    | Option | EN | TH | Notes |
    |--------|----|----|-------|
    | A | [Copy] | [Copy] | [Guideline rule applied] |
    | B | [Copy] | [Copy] | [Guideline rule applied] |
...
## Output Format — Audit
    ...
    **Files scanned**: [X] | **Violations**: [X] | **Score**: [X/100]
    ...
    |------|-------|----------|---------|
    | [Capitalization] | [X] | High/Med/Low | [key: "..." should be "..."] |
...
## Relationship with ux-writing Skill

| Skill | Responsibility |
|-------|---------------|
| **ux-writing** | Universal copy principles + MCP workflow (search, generate, batch) |
| **brand-voice** | Platform-specific rules (capitalization, punctuation, component formats, vocab) |

When both skills are active:
1. **ux-writing** handles the workflow (search existing → generate → batch)
2. **brand-voice** validates the output against guideline rules
3. Final copy passes both universal quality AND brand compliance

## UX Copy MCP Integration

When UX Copy MCP is connected, the `generate_copy` tool already applies these guideline rules server-side (the guideline is loaded as context). Use brand-voice skill to:
- **Double-check** MCP output against rules
- **Audit** existing copy in locale files
- **Review** manually written copy before commit

When MCP is not connected, this skill provides the complete rule set for manual copy writing.
