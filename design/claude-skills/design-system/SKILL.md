...
If the user provides a Figma URL or library, use the Figma MCP to audit components directly — check naming, variants, and token usage. Pull component properties and layer structure for documentation. Use `get_variable_defs` and `get_libraries` to pull token definitions and component inventory.
